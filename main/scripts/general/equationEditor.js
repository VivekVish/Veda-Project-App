...
function equationEditor(targetTextArea,targetDisplayArea,options)
{
	...
	var icons = 
			{	
				...
				//'small_paren'		:	{'image':'\\bigl(\\begin{smallmatrix}\\end{smallmatrix}\\bigr)'	,	'display':'\\bigl(\\begin{smallmatrix}\\end{smallmatrix}\\bigr)'},
	...
	{
		targetDisplayArea.innerHTML = '\\('+targetTextArea.value+'\\)';
	...
	function getElementsByClass(checkedClass, parentElement)
	{
	...
	function addEquationEditorButton(buttonLabel,buttonContainer)
	{
		...
		try
		{
			equationEditorDisplay.innerHTML = '\\('+icons[buttonLabel].image+'\\)';
		}
		catch(e)
		{
	...
	if(typeof(options)!='undefined')
	{
		if(typeof(options.buttons)!='undefined'&&typeof(options.buttonLocation)!='undefined')
		{
			...
			for(var i=0;i<options.buttons.length;i++)
			{
				...
				if(isArray(options.buttons[i]))
				{
					var firstButton = addEquationEditorButton(options.buttons[i][0],rowDiv);
					...
					for(j=1;j<options.buttons[i].length;j++)
					{
						...
						addEquationEditorButton(options.buttons[i][j],nextSpan);
					}
				...
				{
					addEquationEditorButton(options.buttons[i],rowDiv);
				}
			}
			
			var allEquationEditorButtons = getElementsByClass('equationEditorButton',options.buttonLocation);
			
			for(i=0; i<allEquationEditorButtons.length; i++)
			{
				allEquationEditorButtons[i].onclick = function(e)
				{
					var textAreaCaret    = getCaret(targetTextArea);
					var textAreaVal      = targetTextArea.value;
					var buttonName 	     = this.id.substring(this.id.search('-')+1,this.id.length);			

					targetTextArea.value = textAreaVal.slice(0,textAreaCaret[0])+icons[buttonName].display+" "+textAreaVal.slice(textAreaCaret[1],textAreaVal.length);
					
					updateEquation();
					
					var bracePos   = icons[buttonName].display.search('{}');
					var bracketPos = icons[buttonName].display.search('\\[\\]');
					var newTextPosition;
					...
					{
						if(icons[buttonName].display.search('\\\\begin\{.+?\}')>-1)
						{
							newTextPosition = textAreaCaret[0]+icons[buttonName].display.search('}')+1;		
						}
						else
						{
							newTextPosition = textAreaCaret[0]+icons[buttonName].display.length+1;
						}
					}
					
					setSelectionRange(targetTextArea,newTextPosition,newTextPosition);
