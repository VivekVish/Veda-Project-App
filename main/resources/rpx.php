...
$rpx_api_key = '717cc9ac6e750dc5dcb2ffc86acfe4d6f2d07046';
...
//          ),
    ...
    */
		//Get a name from the profile
		if (!empty($auth_info['profile']['preferredUsername'])) {
		  $name = $auth_info['profile']['preferredUsername'];
		} elseif (!empty($auth_info['name']['formattedName'])) {
		  $name = $auth_info['name']['formattedName'];
		} elseif (!empty($auth_info['name']['givenName'])) {
		  $name = $auth_info['name']['givenName'];
		}
		
		session_name('engage');
		session_start();
		$_SESSION['authinfo'] = $auth_info;
		$_SESSION['start'] = time();
		$api = new Api();
